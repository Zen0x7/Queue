<template>
  <div aria-live="assertive" class="pointer-events-none fixed inset-0 flex items-end px-4 py-6 sm:items-start sm:p-6">
    <div class="flex w-full flex-col items-center space-y-4 sm:items-end">
      <transition v-for="notification in notifications.items" :key="notification.id"
        ...
        <div v-if="notification.visible"
          ...
          <div class="p-4">
            <div class="flex items-start">
              <div class="shrink-0">
                <component :is="notification.icon" class="size-6" :class="notification.color" aria-hidden="true"/>
              </div>
              <div class="ml-3 w-0 flex-1 pt-0.5">
                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ notification.title }}</p>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ notification.body }}</p>
              </div>
              <div class="ml-4 flex shrink-0">
                <button type="button" @click="notification.visible = false"
